namespace UIWidgets
{
	...
	public class Sidebar : UIBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IStylable
	{
		#region Interactable
		[SerializeField]
		...
		protected void ModalOpen()
		{
			if (modal && (ModalKey == null))
			{
				...
				var modal_rt = ModalHelper.GetInstance((int)ModalKey).transform as RectTransform;
		...
		protected void ModalClose()
		{
			if (ModalKey != null)
			{
				ModalHelper.GetInstance((int)ModalKey).transform.SetParent(Utilities.FindTopmostCanvas(transform), false);
