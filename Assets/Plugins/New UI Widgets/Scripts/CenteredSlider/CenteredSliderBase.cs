namespace UIWidgets
{
	...
	[DataBindSupport]
	public abstract class CenteredSliderBase<T> : UIWidgetsBehaviour, IPointerClickHandler, IPointerDownHandler, IPointerUpHandler, IStylable, IValidateable
		where T : struct
	{
		/// <summary>
		/// OnChangeEvent
		...
		{
			if (!EqualityComparer<T>.Default.Equals(sliderValue, InBounds(value)))
			{
				sliderValue = InBounds(value);
				UpdateHandle();
				OnValuesChange.Invoke(sliderValue);
				OnChange.Invoke();
			}
		}
		...
		{
			handle = value;
			handle.RectTransform = transform as RectTransform;
		...
		{
			sliderValue = PositionToValue(position - GetStartPoint());
			UpdateHandle();
			OnValuesChange.Invoke(sliderValue);
			OnChange.Invoke();
		}
		...
		protected void UpdateHandle()
		{
			...
			if (IsHorizontal())
			{
				new_position.x = ValueToPosition(sliderValue) + (HandleRect.rect.width * (HandleRect.pivot.x - 0.5f));
			}
			else
			{
				new_position.y = ValueToPosition(sliderValue) + (HandleRect.rect.width * (HandleRect.pivot.x - 0.5f));
			}
		...
		public virtual void OnPointerClick(PointerEventData eventData)
		{
			...
			if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(UsableRangeRect, eventData.position, eventData.pressEventCamera, out cur_position))
			{
				return;
			}

			cur_position -= UsableRangeRect.rect.position;

			var new_position = (IsHorizontal() ? cur_position.x : cur_position.y) + GetStartPoint();
